...
function jsonLoadOneKit($pid): BLLKit
{
    $tkit = new BLLKit();
    $tkit->fromArray(jsonOne("data/json/kits.json", $pid));
    return $tkit;
}
...
function jsonLoadOneNewsItem($pid): BLLNewsItem
{
    ...
    if (! empty($tni->item_href))
    {
        $tni->summary = file_get_contents("data/html/{$tni->item_href}");
    }
    return $tni;
}
...
function jsonLoadAllKit(): array
{
    $tarray = jsonAll("data/json/kits.json");
    return array_map(function ($a) {
        $tc = new BLLKit();
        $tc->fromArray($a);
        return $tc;
